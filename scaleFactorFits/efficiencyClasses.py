...
class efficiency(object):
    ...
    def __init__(self, d_pass, d_fail):
        ...
        #uncomment the following lines to use correlations between nuisance parameters
       # e_pass_tot2 = d_pass.cov_statistical().Integral(1, overflowBin, 1, overflowBin)
       # e_fail_tot2 = d_fail.cov_statistical().Integral(1, overflowBin, 1, overflowBin)
      
       # for key1 in d_pass.d_cov_sys:
       #     for key2 in d_pass.d_cov_sys:   
       #         ex = math.sqrt(d_pass.d_cov_sys[key1].Integral(1, overflowBin, 1, overflowBin))
       #         ey = math.sqrt(d_pass.d_cov_sys[key2].Integral(1, overflowBin, 1, overflowBin))
       #         bin1 = d_pass.correlations.GetXaxis().FindFixBin(key1)
       #         bin2 = d_pass.correlations.GetXaxis().FindFixBin(key2)
       #         corr = d_pass.correlations.GetBinContent(bin1,bin2)
       #         e_pass_tot2 = e_pass_tot2 + (ex*ey*corr)

        #for key1 in d_fail.d_cov_sys:
        #    for key2 in d_fail.d_cov_sys:
        #        ex = math.sqrt(d_fail.d_cov_sys[key1].Integral(1, overflowBin, 1, overflowBin))
        #        ey = math.sqrt(d_fail.d_cov_sys[key2].Integral(1, overflowBin, 1, overflowBin))
        #        bin1 = d_fail.correlations.GetXaxis().FindFixBin(key1)
        #        bin2 = d_fail.correlations.GetXaxis().FindFixBin(key2)
        #        corr = d_fail.correlations.GetBinContent(bin1,bin2)
        #        e_fail_tot2 = e_fail_tot2 + (ex*ey*corr)
