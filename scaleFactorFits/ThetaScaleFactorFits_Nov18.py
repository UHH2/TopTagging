...
def run(fname_stat, fname_sys, outname_stat, outname_sys, calc_sfs = True, write_report = False):
    ...
    inputpath = "thetaFiles_Nov18/mass_sub/"
    ...
    #model_sys = build_model_from_rootfile(inputpath+fname_stat)
    ...
    if '300to400' in fname_sys and 'PUPPI' in fname_sys:
        ...
        if 'wp3' in fname_sys and not '_btag' in fname_sys:
    ...
    PlotPostFitCorrelations(model_stat, mle_output_stat['background_only'], "fitResults_Nov18/nuissance/Corr_"+n_name_stat)
    PlotPostFitCorrelations(model_sys, mle_output_sys['background_only'], "fitResults_Nov18/nuissance/Corr_"+n_name_sys)
   
    writeOutputFile(inputpath+fname_stat,  "fitResults_Nov18/mass_sub/"+outname_stat, mle_output_stat['background_only'], model_stat)
    writeOutputFile(inputpath+fname_sys,  "fitResults_Nov18/mass_sub/"+outname_sys, mle_output_sys['background_only'], model_sys)

    #writeOutputFile("thetaFilesNew+fname,  "fitResults/pt/"+outname, mle_output['background_only'], model, False)
    #writeOutputFile("thetaFiles/tau32/"+fname,  "fitResults/tau32/"+outname, mle_output['background_only'], model, False)
    ...
    if calc_sfs:
        ...
        if not 'HOTVR' in fname_stat:
            ...
            sfs.setMassEffHists("thetaFiles_Nov18/mass_sub/fine/"+fname_stat);
            sfs.setMassEffHistsSys("thetaFiles_Nov18/mass_sub/fine/"+fname_sys);
    ...
    postfit_stat.make_plots("fitResults_Nov18/nuissance/",n_name_stat)

    postfit_sys = ThetaPostFitPlot(mle_output_sys)
    postfit_sys.make_plots("fitResults_Nov18/nuissance/",n_name_sys)
...
for wp in wps: 
    ...
    if calculate_scaleFactors:
        ...
        WriteEffGraphs_separate("ScaleFactors_Nov18/eff_hists_PUPPI"+wp+".root", dicts_stat, dicts_sys, bins)
...
for wp in wpsCHS: 
    ...
    if calculate_scaleFactors:
        dicts_stat = [d_300to400_CHS[0], d_400to480_CHS[0], d_480to600_CHS[0], d_600_CHS[0]]
        dicts_sys = [d_300to400_CHS[1], d_400to480_CHS[1], d_480to600_CHS[1], d_600_CHS[1]]
        WriteEffGraphs_separate("ScaleFactors_Nov18/eff_hists_CHS"+wp+".root", dicts_stat, dicts_sys, bins)

d_300to400_HOTVR = run("thetaFile_300to400_HOTVR_stat_.root", "thetaFile_300to400_HOTVR_sys_.root", "Hists_300to400_HOTVR_stat_.root", "Hists_300to400_HOTVR_sys_.root", calculate_scaleFactors)
d_400to480_HOTVR = run("thetaFile_400to480_HOTVR_stat_.root", "thetaFile_400to480_HOTVR_sys_.root", "Hists_400to480_HOTVR_stat_.root", "Hists_400to480_HOTVR_sys_.root", calculate_scaleFactors)
d_480to600_HOTVR = run("thetaFile_480to600_HOTVR_stat_.root", "thetaFile_480to600_HOTVR_sys_.root", "Hists_480to600_HOTVR_stat_.root", "Hists_480to600_HOTVR_sys_.root", calculate_scaleFactors)
d_600_HOTVR = run("thetaFile_600_HOTVR_stat_.root", "thetaFile_600_HOTVR_sys_.root", "Hists_600_HOTVR_stat_.root", "Hists_600_HOTVR_sys_.root", calculate_scaleFactors)
...
if calculate_scaleFactors:
    ...
    WriteEffGraphs_separate("ScaleFactors_Nov18/eff_hists_HOTVR.root", dicts_stat, dicts_sys, bins)
